...
/**
 * 构建 Phantom signMessage deeplink
 * @see https://docs.phantom.com/solana/signing-a-message
 */
export function openPhantomSignMessageDeeplink(
  message: Uint8Array,
  redirectLink: string,
  phantomEncryptionPublicKey: string,
  dappKeyPair: nacl.BoxKeyPair,
  ...
  try {
    ...
    // 1. 构建 payload (参考官方文档: https://docs.phantom.com/phantom-deeplinks/provider-methods/signmessage)
    const payload = {
      session,
      message: bs58.encode(message),
      display: 'utf8', // 明确指定显示格式
    }

    console.log('[signMessage] Payload:', {
      session: session.substring(0, 20) + '...',
      messageLength: message.length,
      messageBase58: bs58.encode(message).substring(0, 50) + '...',
    })
    ...
    const deeplinkUrl = buildUrl('signMessage', params)
    console.log('Opening Phantom signMessage deeplink', deeplinkUrl)
    ...
    console.error('Error creating signMessage deeplink:', error)
    throw error
  }
}
...
export function decryptSignMessageResponse(
  ...
  if (!decryptedData) {
    throw new Error('Failed to decrypt signMessage response')
  }
